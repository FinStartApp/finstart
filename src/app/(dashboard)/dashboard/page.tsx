...
export default function DashboardPage() {
  ...
  const earnerCalcs = earners.map(e => calculateEarner(e, state_of_residence, filing_status))
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
        {!hasIncome ? (
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-1">Monthly cash flow</p>
              <p className="text-2xl font-bold text-[var(--muted-foreground)]">—</p>
              <p className="text-sm text-[var(--muted-foreground)] mt-1">Add your income to see your financial picture</p>
            </div>
            <Link href="/dashboard/income" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[var(--primary)] text-[var(--primary-foreground)] text-sm font-medium hover:opacity-90 transition-all">
              <Plus size={15} /> Add Income
            </Link>
          </div>
        ) : (
          <div className="flex items-stretch divide-x divide-[var(--border)] w-full">
            {/* Cash flow */}
            <div className="flex flex-col justify-center pr-8 min-w-0 flex-1">
              <p className="text-[11px] font-semibold text-[var(--muted-foreground)] uppercase tracking-widest mb-1.5">Monthly cash flow</p>
              <div className="flex items-center gap-2">
                {cashFlowPositive ? <TrendingUp size={20} className="text-emerald-600 flex-shrink-0" /> : <TrendingDown size={20} className="text-red-500 flex-shrink-0" />}
                <span className={`text-3xl font-bold ${cashFlowPositive ? 'text-emerald-600' : 'text-red-500'}`}>
                  {cashFlowPositive ? '+' : ''}{formatCurrency(pl.net_cash_flow)}
                </span>
              </div>
              <p className="text-xs text-[var(--muted-foreground)] mt-1">{formatCurrency(pl.net_cash_flow * 12)}/yr after expenses &amp; savings</p>
            </div>
            {/* Take-home */}
            <div className="flex flex-col justify-center px-8 min-w-0 flex-1">
              <p className="text-[11px] font-semibold text-[var(--muted-foreground)] uppercase tracking-widest mb-1.5">Take-home</p>
              <p className="text-3xl font-bold text-[var(--foreground)]">{formatCurrency(pl.total_take_home)}</p>
              <p className="text-xs text-[var(--muted-foreground)] mt-1">per month</p>
            </div>
            {/* Expenses */}
            <div className="flex flex-col justify-center px-8 min-w-0 flex-1">
              <p className="text-[11px] font-semibold text-[var(--muted-foreground)] uppercase tracking-widest mb-1.5">Expenses</p>
              <p className="text-3xl font-bold text-[var(--foreground)]">{formatCurrency(pl.total_expenses)}</p>
              <p className="text-xs text-[var(--muted-foreground)] mt-1">Fixed {formatCurrency(pl.total_fixed_expenses)} · Variable {formatCurrency(pl.total_variable_expenses)}</p>
            </div>
            {/* Savings */}
            <div className="flex flex-col justify-center pl-8 min-w-0 flex-1">
              <p className="text-[11px] font-semibold text-[var(--muted-foreground)] uppercase tracking-widest mb-1.5">Savings</p>
              <p className="text-3xl font-bold text-[var(--foreground)]">{formatCurrency(totalSavingsMonthly)}</p>
              <p className="text-xs text-[var(--muted-foreground)] mt-1">per month</p>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-3 gap-4 items-stretch">
        ...
        >
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 h-full">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-semibold text-[var(--foreground)]">Income</h3>
                {anyDeductionsIncomplete && <AlertTriangle size={13} className="text-amber-500" />}
              </div>
              <Link href="/dashboard/income" className="flex items-center gap-1 text-xs text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
                <Pencil size={12} /> Edit
              </Link>
            </div>
            ...
            {!hasIncome ? (
              <div className="space-y-3">
                <p className="text-sm text-[var(--muted-foreground)]">No income added yet</p>
                <Link href="/dashboard/income" className="flex items-center gap-1 text-xs text-[var(--primary)] font-medium">
              ...
              <div className="space-y-2">
                  ...
                  return (
                    <div key={earner.id}>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-[var(--muted-foreground)]">{earner.label}</span>
                        <span className="text-sm font-medium text-[var(--foreground)]">{formatCurrency(totalGross)}</span>
                      </div>
                      {calc.bonus_monthly > 0 && (
                        <div className="ml-2">
                          <span className="text-xs text-[var(--muted-foreground)]">incl. {formatCurrency(calc.bonus_monthly)}/mo bonus</span>
                        </div>
                      )}
                    </div>
                  )
                ...
                <div className="flex justify-between items-center pt-2 mt-1 border-t border-[var(--border)]">
                  <span className="text-sm text-[var(--muted-foreground)]">Taxes &amp; deductions</span>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-[var(--border)]">
                  <span className="text-sm font-semibold text-[var(--foreground)]">Take-home</span>
                  <span className="text-sm font-bold text-[var(--foreground)]">{formatCurrency(income.total_take_home_monthly)}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {incomeHovered && hasIncome && (
            <div className="absolute left-full top-0 w-80 pl-3 z-50">
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-lg p-4">
                <p className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-3">Paycheck breakdown</p>
                {earners.map((earner, i) => {
                  const calc = earnerCalcs[i]
                  const isExpanded = taxExpanded[earner.id]
                  return (
                    <div key={earner.id} className="mb-4 last:mb-0">
                      <p className="text-xs font-semibold text-[var(--foreground)] mb-2">{earner.label}</p>
                      <div className="space-y-1.5">
                        <div className="flex justify-between">
                          <span className="text-xs text-[var(--muted-foreground)]">Gross salary</span>
                          <span className="text-xs font-medium text-[var(--foreground)]">{formatCurrency(calc.gross_monthly)}</span>
                        </div>
                        {calc.bonus_monthly > 0 && (
                          <div className="flex justify-between">
                            <span className="text-xs text-[var(--muted-foreground)]">Bonus (22% supplemental rate)</span>
                            <span className="text-xs font-medium text-[var(--foreground)]">+{formatCurrency(calc.bonus_monthly)}</span>
                          </div>
                        )}
                        ...
                        {calc.bonus_tax_monthly > 0 && (
                          <div className="flex justify-between">
                            <span className="text-xs text-[var(--muted-foreground)]">Bonus tax (22%)</span>
                            <span className="text-xs text-red-500">-{formatCurrency(calc.bonus_tax_monthly)}</span>
                          </div>
                        )}
                        <div>
                          <button onClick={() => toggleTax(earner.id)} className="flex items-center justify-between w-full">
                            <span className="text-xs text-[var(--muted-foreground)]">Est. taxes (salary)</span>
                            <div className="flex items-center gap-1">
                              <span className="text-xs text-red-500">-{formatCurrency(calc.tax_breakdown.total_tax_monthly)}</span>
                              <ChevronDown size={11} className={`text-[var(--muted-foreground)] transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                            </div>
                          </button>
                          {isExpanded && (
                            <div className="mt-1 ml-3 space-y-1">
                              <div className="flex justify-between">
                                <span className="text-xs text-[var(--muted-foreground)]">Federal ({calc.gross_monthly > 0 ? ((calc.tax_breakdown.federal_tax_monthly / calc.gross_monthly) * 100).toFixed(1) : '0.0'}%)</span>
                                <span className="text-xs text-red-400">-{formatCurrency(calc.tax_breakdown.federal_tax_monthly)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-xs text-[var(--muted-foreground)]">State ({state_of_residence || 'N/A'}) ({calc.gross_monthly > 0 ? ((calc.tax_breakdown.state_tax_monthly / calc.gross_monthly) * 100).toFixed(1) : '0.0'}%)</span>
        ...
        >
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 h-full">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-[var(--foreground)]">Expenses</h3>
              <Link href="/dashboard/expenses" className="flex items-center gap-1 text-xs text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
                <Pencil size={12} /> Edit
              </Link>
            </div>
            ...
            {!hasExpenses ? (
              <div className="space-y-3">
                <p className="text-sm text-[var(--muted-foreground)]">No expenses added yet</p>
                <Link href="/dashboard/expenses" className="flex items-center gap-1 text-xs text-[var(--primary)] font-medium">
                    ...
                    {expenseRows.map(row => {
                      ...
                      return (
                        <div
                          ...
                          } ${!isEmpty && (isActive ? 'bg-[var(--hover)]' : 'hover:bg-[var(--hover)]')}`}
                          onMouseEnter={() => {
                        ...
                        >
                          <div className="flex items-center gap-1 min-w-0">
                            <span className="text-sm text-[var(--muted-foreground)] truncate">{row.label}</span>
                            {isSubs && <ChevronRight size={11} className="text-[var(--muted-foreground)] flex-shrink-0" />}
                          </div>
                          <span className={`text-sm font-medium flex-shrink-0 ml-2 ${row.monthly === 0 ? 'text-[var(--muted-foreground)]' : 'text-[var(--foreground)]'}`}>
                ...
                <div className="space-y-1.5">
                  {hoveredRow.lineItems.map((li, i) => (
                    <div key={i} className="flex justify-between items-center">
                      <span className="text-xs text-[var(--muted-foreground)] truncate mr-2">{li.label}</span>
                      <span className="text-xs font-medium text-[var(--foreground)] flex-shrink-0 font-[tabular-nums]">{formatCurrency(li.monthly)}</span>
                    </div>
                  ))}
                  <div className="flex justify-between items-center pt-1.5 border-t border-[var(--border)]">
                    ...
                    <span className="text-xs font-bold text-[var(--foreground)] font-[tabular-nums]">{formatCurrency(hoveredRow.monthly)}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 h-full">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-[var(--foreground)]">Savings</h3>
            <Link href="/dashboard/savings" className="flex items-center gap-1 text-xs text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
              <Pencil size={12} /> Edit
            </Link>
          </div>
          <div className="space-y-2">
            ...
            {totalSavingsMonthly === 0 && (
              <p className="text-sm text-[var(--muted-foreground)]">No savings configured yet</p>
            )}
            <div className="flex justify-between items-center pt-2 border-t border-[var(--border)]">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-[var(--foreground)]">Net Worth</h3>
            <Link href="/dashboard/balance-sheet" className="flex items-center gap-1 text-xs text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
              <Pencil size={12} /> Edit
            </Link>
          </div>
          {!hasAssets ? (
            <div className="space-y-3">
              <p className="text-sm text-[var(--muted-foreground)]">Add your assets and debts to calculate net worth</p>
              <Link href="/dashboard/balance-sheet" className="flex items-center gap-1 text-xs text-[var(--primary)] font-medium">
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-[var(--foreground)]">Savings Rate</h3>
          </div>
          {!hasIncome ? (
            <p className="text-sm text-[var(--muted-foreground)]">Add income to calculate your savings rate</p>
          ) : (
            <div className="space-y-4">
              <div>
                <div className="flex items-baseline justify-between mb-1">
                  <span className="text-xs text-[var(--muted-foreground)]">Total savings rate</span>
                  <span className="text-2xl font-bold text-[var(--foreground)]">{formatPercent(savingsRate)}</span>
                </div>
                ...
                <div className="mt-2 h-1.5 w-full bg-[var(--muted)] rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${Math.min(savingsRate, 100)}%` }} />
                </div>
              ...
              <div className="pt-3 border-t border-[var(--border)]">
                <div className="flex items-baseline justify-between mb-1">
                  <span className="text-xs text-[var(--muted-foreground)]">Retirement savings rate</span>
                  <span className="text-lg font-semibold text-[var(--foreground)]">{formatPercent(retirementRate)}</span>
                </div>
                <p className="text-xs text-[var(--muted-foreground)]">
                  {formatCurrency(retirementSavingsMonthly)}/mo toward retirement · target 15% of gross
                </p>
                <div className="mt-2 h-1.5 w-full bg-[var(--muted)] rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${Math.min(retirementRate / 15 * 100, 100)}%` }} />
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-xs text-[var(--muted-foreground)]">0%</span>
        ...
        >
          <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] w-full max-w-xl max-h-[85vh] flex flex-col shadow-xl">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-[var(--border)]">
              <span className="text-base font-bold text-[var(--foreground)]">Subscriptions &amp; memberships</span>
              <div className="flex items-center gap-4">
                <span className="text-xs text-[var(--muted-foreground)]">
                  Total: <span className="text-[var(--foreground)] font-semibold font-[tabular-nums]">{formatCurrency(subsTotal)} / mo</span>
                </span>
                <button onClick={() => setShowSubsModal(false)} className="text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
                  <X size={16} />
                </button>
              </div>
            </div>
              ...
              {fixed_expenses.subscription_groups.filter(g => g.subscriptions.length > 0).map(group => {
                ...
                return (
                  <div key={group.id} className="border-b border-[var(--border)] last:border-0">
                    <div className="flex items-center justify-between px-5 py-2.5 bg-[var(--secondary)]">
                      <span className="text-[11px] font-bold text-[var(--foreground)] uppercase tracking-wide">{group.name}</span>
                      <span className="text-sm font-bold text-[var(--foreground)] font-[tabular-nums]">{formatCurrency(groupTotal)} / mo</span>
                    </div>
                    <div className="px-5">
                      {group.subscriptions.map(sub => {
                        ...
                        return (
                          <div key={sub.id} className="flex items-center gap-2 py-1.5 border-b border-[var(--muted)] last:border-0">
                            <span className="text-xs text-[var(--foreground)] flex-1">{sub.name || 'Unnamed'}</span>
                            <span className="text-xs text-[var(--muted-foreground)] font-[tabular-nums]">{formatCurrency(amt)} {freqLabel}</span>
                            <span className="text-xs text-[var(--muted-foreground)]">·</span>
                            <span className="text-xs font-medium text-[var(--foreground)] font-[tabular-nums]">{formatCurrency(monthly)}/mo</span>
                          </div>
                        )
                      })}
                    </div>
            ...
            <div className="px-5 py-3 border-t border-[var(--border)] flex justify-between items-center">
              <Link href="/dashboard/expenses" onClick={() => setShowSubsModal(false)} className="text-xs text-[var(--accent)] hover:underline">
                Edit subscriptions →
              </Link>
              <button onClick={() => setShowSubsModal(false)} className="bg-[var(--primary)] text-[var(--primary-foreground)] text-xs font-semibold px-4 py-1.5 rounded-lg hover:opacity-90 transition-opacity">
                Done
              </button>
            </div>
          </div>
